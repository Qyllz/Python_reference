...
def create_board():
    board = np.zeros((ROW_COUNTS, COLUMNS_COUNTS))
    return board
...
def winning_move(board, piece):
    for c in range(COLUMNS_COUNTS - 3):
        for r in range(ROW_COUNTS):
            if (
                    board[r][c] == piece and
...
width = COLUMNS_COUNTS * SQUARE_SIZE
height = (ROW_COUNTS + 1) * SQUARE_SIZE
size = (width, height)
            ...
            if turn == 0:
                ...
                if valid_location(board, col):
                    ...
                    if winning_move(board, 1):
                        ...
                        break  # Exit the event loop when the game is over
